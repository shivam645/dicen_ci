...
class UserModel extends CI_Model
{
    ...
    public function getHistory($userId, $userUrl, $getPag = 0, $limit = 5) {
        ...
        foreach ($query->result() as $row) {
            ...
            // if($row->type = 1) {
            //     $row->status = $this->updateStripeStatus($row->subscription_id); //<td >'.$row->subscription_id.'</td>
            // }
        ...
        try {
            ...
            if($typeSubscription === '1') {
                $subsription = \Stripe\Subscription::create(array(
                    "customer" => $customerId,
                    "items" =>  [
                            ['price' => 'price_1KXSBfH1NghxxqJZnGxpoaUK'],
                          ]
                ...
                $subsription = \Stripe\Charge::create(array(
                    'customer' => $customerId,
                    'plan'   => str_replace(".", "", $this->config->item('payPrice')),
            ...
            /*echo '<pre>';print_r($e);print_r(array(
                    'customer' => $customerId,
                    'amount'   => $this->config->item('plan'),
                    'currency' => $this->config->item('payCurrency')
                ));echo '<pre>';*/
            return alert($this->lang->line('An error has occurred, payment has not been send').' : '. $e->getMessage(), 'danger');
            exit;
        }
